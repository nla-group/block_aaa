...
%
% with addition of (moderate) normally distributed NOISE 
...
npts = 200;  % number of sampling points
pts = logspace(-1,2,npts)*1i;

load iss

% enforce the symmetry of the TF
C = B.';

Nl = length(A);

% dimension of the underlying rational matrix  function
m = size(C,1);
n = size(B,2);
...
for i = 1:npts
    FF{i} = full(C*((pts(i)*speye(Nl)-A)\B))+nlevel*randn(m,n);
end

maxell = 80; % maximum degree tried for all methods
timingell = 20; % degree for which to time each of the methods
...
axis tight
ylim([10^(-2) 10^(3)]);
%title('The ISS example with noise (size 3x3)')
title(['ISS example with noise'])
...
yt={'10^{-2}'; '10^{-1}';'10^{0}'; '10^{1}'; '10^{2}';'10^{3}'}; 
