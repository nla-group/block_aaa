% compare all fitting algorithms on Example 2 from the MFToolbox
%
...
load ex2_Y  % 302 points, but weirdly the last first/one is doubled
FF = cell(1,300);
for i = 2:301
    FF{i-1} = bigY(:,:,i);
end

npts = 300;  % number of sampling points
pts = s(2:301);
maxell = 50; % maximum degree tried for all methods
...
for i = 1:3
    for j = 1:3
        semilogy(imag(s),abs(squeeze(bigY(i,j,:)))), hold on
    end
end
axis tight, ylim([1e-5,1e-1])
xlabel('$z/i$')
ylabel('$|F_{i,j}(z)|$')
title('MF Toolbox example')
