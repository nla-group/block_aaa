% compare all fitting algorithms on the Bucking Plate NonLinEig problem
...
% F = @(z) [ (z*(1 - 2*z*cot(2*z))/(tan(z)-z))+10, z*(2*z-sin(2*z))/(sin(2*z)*(tan(z)-z)),  2;
%     z*(2*z-sin(2*z))/(sin(2*z)*(tan(z)-z)), (z*(1 - 2*z*cot(2*z))/(tan(z)-z))+4, 2;
%     2, 2, 8];
  
F = @(z) [ (z*(1 - 2*z*cot(2*z))/(tan(z)-z))+10, z*(2*z-sin(2*z))/(sin(2*z)*(tan(z)-z));
    z*(2*z-sin(2*z))/(sin(2*z)*(tan(z)-z)), (z*(1 - 2*z*cot(2*z))/(tan(z)-z))+4];

npts = 500;
pts = exp(2i*pi*(1:npts)/npts);        % sampling points
...
maxell = 30; % maximum degree tried for all methods
